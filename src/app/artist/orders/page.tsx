...
export default async function ArtistOrdersPage() {
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <div>
        <p className="font-mono text-[11.5px] uppercase tracking-[0.16em] text-ink-soft">Revenue</p>
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-md border border-border bg-surface py-16 text-center">
          <p className="font-display text-[17px] font-semibold text-ink">No sales yet.</p>
        ...
        <>
            ...
            <table className="w-full text-[13px]">
              <thead className="border-b border-border bg-muted/50">
                <tr>
                  ...
                  <th className="hidden px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[0.12em] text-ink-soft lg:table-cell">Date</th>
                  <th className="px-4 py-3 text-right font-mono text-[10px] uppercase tracking-[0.12em] text-ink-soft">Earnings</th>
                  <th className="px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[0.12em] text-ink-soft">Status</th>
                  <th className="px-4 py-3" />
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {orders.map((order) => {
                  const earnings = order.items.reduce((s, i) => s + Number(i.unitPrice) * i.quantity * 0.85, 0);
                  return (
                    <tr key={order.id} className="transition-colors hover:bg-muted/30">
                      <td className="px-4 py-3.5 font-mono text-[12px] tabular-nums text-ink">{order.orderNumber}</td>
                      <td className="hidden px-4 py-3.5 text-ink-muted md:table-cell">{order.buyer.name}</td>
                      <td className="hidden px-4 py-3.5 text-ink-soft lg:table-cell">
                        {order.paidAt ? format(order.paidAt, "d MMM yyyy") : "—"}
                      </td>
                      <td className="px-4 py-3.5 text-right font-mono tabular-nums text-ink">
                        {formatCurrency(earnings, "GHS")}
                      </td>
          ...
          <ul className="space-y-3 sm:hidden">
            {orders.map((order) => {
              const earnings = order.items.reduce((s, i) => s + Number(i.unitPrice) * i.quantity * 0.85, 0);
              return (
                <li key={order.id} className="rounded-md border border-border bg-surface p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[12px] tabular-nums text-ink">{order.orderNumber}</span>
                    <span className={`inline-flex items-center rounded-sm border px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.1em] ${statusStyle[order.status] ?? "border-border text-ink-soft"}`}>
                      {order.status.toLowerCase()}
                    </span>
                  </div>
                  <p className="text-[13px] text-ink-muted">{order.buyer.name}</p>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[13px] tabular-nums text-ink">{formatCurrency(earnings, "GHS")}</span>
